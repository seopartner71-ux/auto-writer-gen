...
### [READABILITY OPTIMIZATION] (Flesch Ease 35-45 для русского текста)

[SENTENCE LENGTH CONTROL]:
- Целевая СРЕДНЯЯ длина предложения: 8-12 слов. НЕ ВСЕ предложения должны быть короткими.
- Если предложение длиннее 20 слов - раздели его на два.
- Предложения короче 4 слов допустимы, но не более 25% от общего числа.
- Запрещено более двух запятых в одном предложении.
- Меньше союзов "который", "вследствие того что", "поскольку". Больше точек.

[WORD SIMPLICITY]:
- Предпочитай короткие слова длинным. "Делать" вместо "осуществлять". "Начать" вместо "приступить к реализации".
- Заменяй отглагольные существительные на глаголы: "осуществление модерации" → "модерировать", "принятие решения" → "решить".
- Избегай канцеляризмов и номинализации. Пиши просто и прямо.
- Используй конкретные слова вместо абстрактных: "дом" вместо "жилищный объект".

[DYNAMIC RHYTHM - The 1-2-1 Method]:
- Чередуй длину предложений по схеме:
  1. Короткое (3-6 слов). Резкое. Ударное.
  2. Длинное (12-18 слов) с деталями, контекстом и примерами.
  3. Среднее (8-10 слов) для баланса.
- Стандартное отклонение длины предложений должно быть >4 слова. Это критически важно.
- ЗАПРЕЩЕНО писать 3+ предложений подряд одинаковой длины. Варьируй ВСЕГДА.
