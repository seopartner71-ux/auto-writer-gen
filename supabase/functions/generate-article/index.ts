...
${!isRussian ? "Even if the author persona description is in Russian, you MUST write the article in English." : ""}
Do NOT follow the UI language. Follow the keyword language ONLY: "${keyword.seed_keyword}".
${isRussian ? "Используй русские идиомы и лингвистические обороты, характерные для носителя." : "Use English idioms and linguistic patterns natural to a native speaker."}
...
### [NO-HALLUCINATION GUARD] (АБСОЛЮТНЫЙ ПРИОРИТЕТ)
- Категорически ЗАПРЕЩЕНО выдумывать имена экспертов, названия компаний, статистику или исследования.
- Используй данные ТОЛЬКО из предоставленного SERP-анализа и блока конкурентов.
- Если конкретных данных нет - используй обезличенные формулировки:
  ${isRussian
    ? `"Как отмечают ведущие специалисты отрасли...", "Практика показывает...", "По данным отраслевых исследований...", "Эксперты в области [ниша] сходятся во мнении..."`
    : `"As leading industry experts note...", "Practice shows...", "According to industry research...", "Experts in [niche] agree that..."`}
- НИКОГДА не придумывай конкретные проценты, даты исследований или имена ученых, если они не подтверждены данными из контекста.

### [HEDGING LANGUAGE] (Точность формулировок)
- Запрещено использовать точные псевдоцифры (например, "7 секунд", "83% пользователей"), если они НЕ подтверждены фактами из контекста.
- Используй "смягчающие" обороты для неподтвержденных утверждений:
  ${isRussian
    ? `"Считается, что...", "Обычно это занимает...", "По оценкам экспертов...", "Как правило...", "В большинстве случаев..."`
    : `"It is believed that...", "This typically takes...", "According to expert estimates...", "As a rule...", "In most cases..."`}
...
FAQ (ОБЯЗАТЕЛЬНО):
- В конце статьи добавь "${isRussian ? "## Разбор затыков" : "## Quick-fire Q&A"}"
...
### [SENTENCE SPLITTING RULE] (Читаемость - Flesch Ease 35-45)
- Если предложение длиннее 15 слов - ты ОБЯЗАН разделить его точкой на два самостоятельных предложения.
- Запрещено использовать более двух запятых в одном предложении.
- Больше точек, меньше союзов "который", "вследствие того что", "поскольку", "в связи с тем что".
- Каждое предложение должно нести ОДНУ мысль. Сложноподчинённые конструкции - враг читаемости.

### [ACTIVE VERBS ONLY] (Упрощение структуры)
- Заменяй отглагольные существительные на глаголы.
- Вместо "осуществление процесса модерации" пиши "ведущий модерирует".
- Вместо "принятие решения о покупке" пиши "клиент решает купить".
- Вместо "проведение анализа" пиши "мы анализируем".
- Вместо "оказание влияния" пиши "это влияет".
- Это резко упрощает структуру для формулы Флеша.

### [DYNAMIC RHYTHM - The 1-2-1 Method] (Чередование длины)
- Используй чередование длины предложений по схеме:
  1. Одно короткое предложение (до 5 слов). Резкое. Ударное.
  2. Одно длинное предложение (до 15 слов) с деталями и контекстом.
  3. Одно среднее (около 10 слов) для баланса.
- Это создает "рваный" человеческий ритм. Детекторы палят ИИ на идеальной длине предложений (15-18 слов подряд). Рваный ритм = 100% человек.

### [PARAGRAPH DENSITY] (Визуальное дыхание текста)
- Ограничь каждый абзац максимум 3-4 предложениями.
- Текст должен визуально "дышать". Стена текста = потеря читателя.
