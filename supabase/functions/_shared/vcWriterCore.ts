...
/**
 * Уровень воронки контента. Меняет НЕ формат (guide/rating/...), а ИНТЕНТ:
 *  - tofu — охват и трафик из Яндекса, объяснение темы, БЕЗ оффера и без client_links.
 *  - mofu — главный «деньговый» уровень: сравнение, ошибки покупателей, чек-лист выбора, мягкий нативный CTA.
 *  - bofu — ближе к заявке: цены/диапазоны, усиленные ошибки, блок «как мы помогаем», нативный, но более прямой CTA.
 *  - auto — без жёстких правил воронки (старое поведение).
 */
export type FunnelStage = "auto" | "tofu" | "mofu" | "bofu";

export function isFunnelStage(v: unknown): v is FunnelStage {
  return v === "auto" || v === "tofu" || v === "mofu" || v === "bofu";
}

export const FUNNEL_STAGE_RULES: Record<Exclude<FunnelStage, "auto">, string> = {
  tofu: `УРОВЕНЬ ВОРОНКИ: TOFU (трафик / охват / прогрев).
ЦЕЛЬ: попасть в Яндекс и Google по информационным запросам, объяснить тему, БЕЗ продажи.
ОБЯЗАТЕЛЬНО:
- Заголовок информационный ("Что такое X", "Как работает X", "Где применяется X"), без боли и без "как выбрать".
- Вступление простое: что это, зачем, где применяется. БЕЗ "рынок изменился", БЕЗ цифр потерь.
- Основная часть — объяснение блоками + 3-5 сценариев применения.
- Короткий FAQ из 3-5 вопросов уровня "что это?", "сколько стоит примерно?", "кому подойдёт?".
ЗАПРЕЩЕНО:
- Сравнение брендов "Китай vs Япония", таблицы выбора, чек-листы покупателя — это уровень MOFU.
- Любой коммерческий CTA, оффер автора, блок "как мы помогаем", блок "оставить заявку".
- Конкретные цены отдельных брендов и блок "сколько стоит и как не переплатить" — это BOFU.
- Драматичный лид про "переплату 300 000 ₽" и "ошибки покупки".
МОСТИК в конце (вместо CTA): одна фраза "в следующих материалах разберём, как выбрать <X> под конкретную задачу" — без ссылок.`,
  mofu: `УРОВЕНЬ ВОРОНКИ: MOFU (выбор / сравнение / основной поток заявок).
ЦЕЛЬ: читатель уже знает тему и сравнивает варианты — провести его через сравнение к мягкому нативному CTA.
ОБЯЗАТЕЛЬНЫЕ СЕКЦИИ (в этом порядке после лида):
1. Контекст рынка: одно H2 "Почему всё изменилось" (или близкое по смыслу) — что поменялось за год-два, без выдуманных процентов.
2. Сравнение вариантов: H2 со сравнением 3-5 типов решений / стран-производителей / категорий ("Китай vs Индия vs Япония"), у каждого варианта плюсы и минусы и архетип покупателя.
3. H2 "Ошибки покупателей" — 4-6 конкретных ошибок (переплата, неверная мощность/комплектация, нет сервиса, скрытые издержки). Это критично — без этой секции материал не MOFU.
4. H2 "Чек-лист выбора" — 5-7 пунктов формата "<параметр> -> на что смотреть" (задача, площадь/объём, навеска/комплектация, мощность/ресурс, сервис, гарантия, документы).
5. Мягкий мостик в финале: "если не хочется попасть в одну из этих ошибок — вот следующий шаг".
ЗАПРЕЩЕНО: жёсткие продающие фразы ("купите сейчас", "оставьте заявку, менеджер свяжется"), цены конкретных моделей вне диапазонов, выдуманные цифры экономии.
По умолчанию стиль оффера = native CTA, если пользователь подключил блок-оффер.`,
  bofu: `УРОВЕНЬ ВОРОНКИ: BOFU (горячий трафик / прямые заявки).
ЦЕЛЬ: читатель уже выбирает подрядчика/поставщика — провести его в заявку.
ОБЯЗАТЕЛЬНЫЕ СЕКЦИИ:
ЗАГОЛОВОК (КРИТИЧНО): обязательно содержит маркер денег/риска/года — цена, "не переплатить", "где теряют", "ошибки выбора", год в формате 2026. Шаблоны: "Сколько на самом деле стоит <X> в 2026 и где теряют 30% бюджета", "Как выбрать <X> и не переплатить: ошибки на сотни тысяч", "<X> в 2026: реальная цена, скрытые расходы и как не купить не то".
1. ЛИД (5-8 строк, жёсткий хук, СТРОГАЯ структура трёх движений): (а) что изменилось на рынке за 2025-2026 — БЕЗ выдуманных процентов; (б) почему покупатели ошибаются — ориентируются на старые цены / советы друзей / первое попавшееся КП; (в) цена ошибки в деньгах или в риске простоя/переделки. Год обязательно в заголовке или в первой строке лида.
2. H2 "Сколько это стоит в <год>" — БЛОК "БОЛЬ + ДЕНЬГИ". Обязательно ценовые ДИАПАЗОНЫ по сегментам (бюджет / средний / премиум). Минимум 1-2 раза в блоке использовать формулу "люди думают, что <X>, но на самом деле <Y>" — это создаёт когнитивный разрыв и удерживает внимание. Если конкретных цен в проверенных фактах нет — диапазон словами ("в среднем от X до Y тыс. руб по рынку"), но сегментация обязательна.
3. H2 "Разбор вариантов" (или "Что вообще можно купить за эти деньги") — 3 уровня решений: дешёвый / средний / профи. У КАЖДОГО варианта обязательная пара "плюс -> риск" ("дешёвый -> риск переделки / простоя / отсутствия сервиса"; "средний -> компромисс по ресурсу"; "профи -> переплата за бренд если задача не та"). Без энциклопедии — 2-4 строки на вариант.
4. H2 "Где теряют деньги" (или "Ошибки, на которых теряют деньги") — 4-6 усиленных ошибок (неверная мощность/класс, переплата за бренд под чужую задачу, серые дилеры, отсутствие сервиса/гарантии, скрытая навеска, недооценённые расходники). Цель блока — создать страх ошибки.
5. H2 "Чек-лист правильного выбора" — 5-7 пунктов СТРОГО по осям: задача -> нагрузка/режим -> условия эксплуатации -> бюджет (с диапазоном) -> сервис и гарантия -> документы/комплектация. Формат "<ось> -> на что смотреть". Цель — ощущение "я почти понял, но мне нужна помощь со своим случаем".
6. МОСТ к решению (1 короткий абзац, 2-3 предложения, БЕЗ H2): нативно связывает чек-лист и блок "Как мы помогаем". Шаблон: "На практике большинство ошибок можно избежать ещё до покупки — если правильно разобрать задачу и подобрать конфигурацию под реальные условия". Без моста переход к "Как мы помогаем" выглядит как реклама.
7. H2 "Как мы помогаем" (или близкое по смыслу) — короткий блок 60-100 слов в КОНСАЛТИНГ-ТОНЕ: разбираем задачу, считаем варианты, показываем где можно не переплачивать. НЕ "мы продаём", НЕ "наша компания лидер". Формула: "не продаём — разбираем".
8. Нативный CTA (если подключён offer_block — он сюда логически встраивается): мягкая формулировка из whitelist ("если вы сейчас на этапе выбора — можно разобрать ваш случай", "напишите задачу — подскажем какой вариант реально подходит под ваши условия без переплаты за лишние характеристики"). Если offer_block не подключён — блок "Как мы помогаем" всё равно ОБЯЗАТЕЛЕН, но без внешней ссылки.
9. Финал — закрывающий вопрос в P.S. формата "А вы уже определились с вариантом или пока сравниваете?" / "Что для вас важнее — цена или ресурс?". Цель — комментарии и удержание.
По умолчанию стиль оффера = native CTA. Если у пользователя подключён блок-оффер — он становится логическим продолжением блока "как мы помогаем". Если оффер не подключён — блок "как мы помогаем" всё равно ОБЯЗАТЕЛЕН, но без внешней ссылки.
ЗАПРЕЩЕНО: "успейте", "акция", "скидка 50%", "только сегодня", восклицательные знаки в CTA, выдуманные кейсы "сэкономили клиенту 1.8 млн", энциклопедические разборы вариантов на 5+ абзацев каждый, прямое "купите/закажите/оставьте заявку и менеджер свяжется".`,
};
...
ЖАНР (КРИТИЧНО): это НЕ «каталог компаний», а «независимое расследование рынка». Тон — суровый практик-снабженец/инженер, который потерял деньги на плохом поставщике и пошёл искать нормального. Аудитория VC ненавидит рекламные подборки и любит расследования с цифрами потерь.

H1 (ОБЯЗАТЕЛЬНО, первая строка markdown, формат "# <текст>"): продаёт ИСТОРИЮ ПОТЕРИ денег/времени + факт исследования, а не список брендов. Хороший пример: «Из-за одного порванного РВД экскаватор простоял 4 дня. Мы проверили 8 поставщиков в Москве - вот кто гонит брак». Плохой пример (БРАК): «Рейтинг производителей РВД в Москве - данные 2026». Если пользователь явно задал шаблонное название - переформулируй в формат расследования, но сохрани ключевые слова (регион, ниша, год). Год обязательно где-то в H1 или в первой строке после него.

ИНТРИГА (КРИТИЧНО): победитель НЕ называется в лиде и в первой трети текста. Никаких «по нашему опыту лучший - <бренд>» во вступлении. Имя бренда-победителя впервые появляется только в нумерованном списке участников и в его карточке №01. Если PINNED_COMPANY задан - он всё равно стоит на позиции 01, но в лиде не упоминается. Лид рассказывает про БОЛЬ и МЕТОД, а не про результат.
...
Затем МЕТОДОЛОГИЯ (1 абзац, 2-3 предложения): по каким 3-4 критериям сравнивали и КАК собирали данные (тестовые заказы, прозвоны, опрос коллег по рынку, открытые отзывы). Критерии всегда конкретные для ниши: например «качество опрессовки / соблюдение сроков / клиентский сервис / цена за единицу». Каждый критерий оценивается по шкале 1-10 и попадает в карточки.
...
Оценки: качество <X>/10 · сроки <Y>/10 · сервис <Z>/10 · ИТОГ <сумма или средневзвешенное>/10

БАЛЛЬНАЯ МЕТОДИКА (КРИТИЧНО): строка "Оценки: ... · ИТОГ N/10" обязательна в каждой карточке (плоским текстом, отдельной строкой после буллетов, БЕЗ markdown-таблицы - VC её не рендерит). Топ-1 (или PINNED_COMPANY на позиции 01) ОБЯЗАН выигрывать МАТЕМАТИЧЕСКИ: его ИТОГ строго выше всех остальных. Оценки конкурентов распределяй реалистично (диапазон 5.0-9.0, без круглых 10/10 у всех) - топ-1 не должен быть «10 везде», иначе видна подгонка. Победитель убедителен балансом, а не максимумом по всем строкам.
...
- АРХЕТИПЫ КОНКУРЕНТОВ (КРИТИЧНО для разнообразия): у каждого конкурента (не топ-1) должен быть СВОЙ ЖИВОЙ «характер» - один реалистичный системный минус, который видно в зоне роста и отражается в баллах. Набор архетипов: «дёшево, но срывают сроки на неделю», «премиум-цена, но менеджеры не отвечают сутками», «дешёвые китайские комплектующие - текут через месяц», «хамоватые менеджеры на входящих», «нет склада редких позиций - ждать 2-3 недели», «качество ок, но минимальный заказ от X тыс. руб». Запрещено двум конкурентам иметь одинаковый архетип провала. Топ-1 - выигрывает балансом (нет жирного минуса, но и не «идеален»).
- ЖИВОЙ ЯЗЫК (КРИТИЧНО): в буллетах и зонах роста - конкретные фразы практика, а не корпоративный булшит. ПЛОХО: «оперативное реагирование», «индивидуальный подход», «конкурентная цена». ХОРОШО: «ответили через 15 минут», «трубку не брали полтора дня», «рукав потёк через неделю», «привезли за 2 часа, но без половины позиций».
...
function buildPrompt(p: VcGenInput): { system: string; user: string } {
  ...
  const persona = p.authorPersona && AUTHOR_PERSONA_BRIEF[p.authorPersona]
    ...
    : "";
  // Полный авторский профиль (system_prompt + тон + примеры стиля + стоп-слова).
  // Если передан — добавляется как высокоприоритетный блок ПОВЕРХ persona-брифа.
  const customAuthorBlock = (() => {
    const ca = p.customAuthor;
    if (!ca) return "";
    const parts: string[] = [];
    if (ca.name) parts.push(`Имя автора: ${ca.name}`);
    if (ca.voiceTone) parts.push(`Тон/голос: ${ca.voiceTone.trim().slice(0, 400)}`);
    if (ca.systemPrompt && ca.systemPrompt.trim()) {
      parts.push(`Авторский system prompt (ВЫШЕ ВСЕХ стилевых правил vc.ru, кроме фактологии и численного гарда):\n${ca.systemPrompt.trim().slice(0, 6000)}`);
    }
    if (ca.styleExamples && ca.styleExamples.trim()) {
      parts.push(`Примеры стиля автора (имитируй ритм, синтаксис, словарь — НЕ копируй смысл):\n${ca.styleExamples.trim().slice(0, 2000)}`);
    }
    if (ca.negativeInstructions && ca.negativeInstructions.trim()) {
      parts.push(`ЗАПРЕТЫ автора (нарушение = брак):\n${ca.negativeInstructions.trim().slice(0, 1500)}`);
    }
    if (Array.isArray(ca.stopWords) && ca.stopWords.length) {
      const sw = ca.stopWords.map((w) => String(w || "").trim()).filter(Boolean).slice(0, 60);
      if (sw.length) parts.push(`Стоп-слова автора (НЕ используй ни в каком виде): ${sw.map((w) => `"${w}"`).join(", ")}`);
    }
    if (!parts.length) return "";
    return `\n\nАВТОРСКИЙ ПРОФИЛЬ (КРИТИЧНО, держи голос автора):\n${parts.join("\n\n")}`;
  })();
  const niche = (p.nicheTerms && p.nicheTerms.length)
    ...
    : "";
  const offer = (() => {
    const o = p.offerBlock;
    if (!o || !o.offer || !o.url || !o.cta) return "";
    const styleBrief: Record<OfferBlockInput["style"], string> = {
      soft: `СТИЛЬ "SOFT" (вшить в P.S., 60-90 слов):\n- P.S. начинается с короткой человеческой фразы, БЕЗ слов "акция", "купите", "успейте", "только сегодня".\n- Сначала вопрос аудитории (как обычный P.S.), потом 1-2 предложения: "если разбираться лень - <offer>: <benefit>. <cta> -> [анкор](url)".\n- НЕ создавай отдельный H2 в этом стиле. Оффер живет внутри P.S.`,
      native: `СТИЛЬ "NATIVE CTA" (отдельный H2 в самом конце, 80-120 слов, ПОСЛЕ всех остальных секций, но ПЕРЕД P.S.):\n- Заголовок H2: "Что делать дальше" или "Если хочется ускорить" (выбери по смыслу темы, без "купить", "заказать").\n- Структура блока: 1) проблема читателя одной фразой ("если самому разбираться долго / нет времени тестировать / уже обожглись"), 2) что предлагается (<offer>), 3) выгода (<benefit>), 4) одна строка с CTA: "<cta> -> [анкор](url)".\n- Ровно 1 ссылка в блоке. Анкор = "${o.cta}" или близкая фраза.`,
      leadmagnet: `СТИЛЬ "LEAD-MAGNET" (отдельный H2 в самом конце, 80-120 слов, ПЕРЕД P.S.):\n- Заголовок H2: "Забрать <название материала>" или "Полезное в подарок".\n- Сначала 1-2 строки: что внутри материала (чек-лист / расчет / шаблон) и кому пригодится.\n- В конце CTA-строка: "<cta> -> [анкор](url)". Анкор обыгрывает суть подарка ("забрать чек-лист", "получить расчет"), не "перейти на сайт".\n- НЕ обещай того, чего нет в <offer>. Подарок = <offer>.`,
    };
    const disclosure = o.disclosure
      ? `\n- РАСКРЫТИЕ (КРИТИЧНО для rating): прямо в блоке оффера короткой фразой указать, что автор работает/сотрудничает с этой компанией. Пример: "Раскрытие: автор работает с этой компанией - поэтому могу сразу позвать на бесплатный аудит". Без этой фразы блок невалиден.`
      : "";
    return `\n\nБЛОК-ОФФЕР АВТОРА (КРИТИЧНО, ОБЯЗАТЕЛЬНО ВСТАВИТЬ ровно ОДИН раз):\nЦель блока - дать читателю простой путь к заявке, но НЕ выглядеть как баннер. Это НЕ продажа, это логическое продолжение статьи: читатель дочитал, узнал боль, увидел решение - вот следующий шаг.\n\nИСХОДНЫЕ ДАННЫЕ:\n- offer (что предлагается): "${o.offer}"\n- benefit (выгода): "${o.benefit || "(не указано — придумай короткую конкретную выгоду из контекста темы, без воды и без чисел, если их нет в проверенных фактах)"}"\n- cta (текст призыва): "${o.cta}"\n- url (ссылка, уже с UTM): ${o.url}\n\n${styleBrief[o.style]}\n\nКОНТЕКСТНАЯ ПРИВЯЗКА (КРИТИЧНО, без этого блок не работает):\n- В первом предложении блока ОБЯЗАТЕЛЬНО сослаться на конкретную боль/ситуацию, которая УЖЕ упоминалась в теле статьи. Не "если у вас задача X" в вакууме, а "вернёмся к ситуации из <название блока/кейса выше>" или "если узнали себя в истории про <конкретная деталь из статьи>".\n- Оффер должен звучать как естественный следующий шаг для читателя, который согласился с проблематикой материала. Если такой связки нет - блок переписать.\n\nКОНСАЛТИНГ-СТИЛЬ (vc.ru любит разбор, не продажу):\n- МОЖНО: "если столкнулись с этой задачей", "можем разобрать ваш случай", "мы делаем такой разбор", "если хочется второго мнения", "если интересно посмотреть на ваш кейс".\n- НЕЛЬЗЯ: "мы компания X", "мы продаём", "наш сервис лучший", "оставьте заявку и менеджер свяжется", "переходите на сайт".\n\nЖЕЛЕЗНЫЕ ЗАПРЕТЫ (любое нарушение = блок переписать):\n- НЕ использовать слова: "купите", "закажите прямо сейчас", "акция", "скидка", "успейте", "ограниченное предложение", "только сегодня", "оставьте заявку и получите".\n- НЕ использовать восклицательные знаки в блоке.\n- НЕ обещать сроков, цен, гарантий, процентов и любых чисел, которых нет в проверенных фактах.\n- НЕ вставлять оффер ВНУТРИ карточек рейтинга, ВНУТРИ кейса, ВНУТРИ FAQ. Только в конце.\n- Ровно 1 (одна) ссылка в блоке оффера. Никаких дублей, никаких "ещё подробнее тут".\n- Не использовать клиентские ссылки (clientLinks) внутри блока оффера - они работают в теле статьи, а оффер - отдельная единица.${disclosure}\n\nКАК ДОЛЖЕН ВЫГЛЯДЕТЬ CTA-фрагмент:\n- "${o.cta} -> [${o.cta}](${o.url})" - оставь стрелку и ссылку именно в таком формате одной строкой. URL вставить как есть, с UTM-метками - НЕ обрезать, НЕ менять.\n- Между текстом блока и CTA-строкой - пустая строка.`;
  })();
