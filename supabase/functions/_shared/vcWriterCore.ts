...
ОБЯЗАТЕЛЬНЫЕ СЕКЦИИ:
1. Лид: боль + деньги (риск переплаты, простой техники, потеря заказа). Год обязательно в заголовке или первой строке.
2. H2 "Сколько это стоит в <год>" — реальные ценовые ДИАПАЗОНЫ по сегментам (бюджет / средний / премиум). Если конкретных цен нет в проверенных фактах — давай диапазон словами ("в среднем от X до Y тыс. руб по рынку"), но обязательно сегментация.
3. H2 "Ошибки, на которых теряют деньги" — 4-6 усиленных ошибок (серые дилеры, отсутствие гарантии, скрытая навеска, фейковый сервис).
4. H2 "Чек-лист проверки перед покупкой/заказом" — 5-7 пунктов "что спросить", "что проверить в договоре", "что потребовать в комплекте".
5. H2 "Как мы помогаем" (или близкое по смыслу) — короткий блок 60-100 слов: подбор, консультация, комплектация. Без хвастовства, в консалтинг-тоне.
6. Финал — закрывающий вопрос вида "что для вас важнее — цена или ресурс?" в P.S.
...
ЗАПРЕЩЕНО: "успейте", "акция", "скидка 50%", "только сегодня", восклицательные знаки в CTA, выдуманные кейсы "сэкономили клиенту 1.8 млн".`,
};
