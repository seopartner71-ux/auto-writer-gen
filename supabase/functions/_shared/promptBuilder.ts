// Shared prompt builder used by generate-article AND bulk-generate.
// CRITICAL: this is a code MOVE — behaviour must remain identical to the
// original implementation in supabase/functions/generate-article/index.ts.
// All Stealth Protocol, Miralinks/GoGetLinks blocks, Persona logic and
// SEO/E-E-A-T directives live here so single and bulk pipelines share them.
...
2. ОБЯЗАТЕЛЬНО склоняй ключевые слова по падежу, числу и роду так, чтобы фраза читалась грамотно. Грамматика русского языка важнее дословного совпадения — не вставляй ключ в исходном падеже, если это ломает согласование ("один круглые стикеры" — недопустимо; правильно: "один круглый стикер").
...
- После H1 ОБЯЗАТЕЛЬНО разбей статью на смысловые разделы через Markdown-заголовки H2: строки должны начинаться строго с "## ". Подразделы оформляй строками "### ". Нельзя писать заголовки обычным жирным текстом, строками без # или HTML-тегами.
- Минимум 4 H2-раздела в каждой статье, если Блок Б не задает более подробную структуру. В правой панели статьи H2/H3 считаются только по строкам "## " и "### ", поэтому эти маркеры обязательны.
...
- ТОПОНИМЫ (города, регионы, страны) ВСЕГДА с заглавной буквы в ЛЮБОМ падеже, как в заголовках, так и в теле статьи: Москва / Москвы / в Москве, Тула / Туле / в Туле, Санкт-Петербург, СПб, Калуга, Рязань, Ярославль, Тверь, Сочи, Россия. Прилагательные, образованные от топонимов, тоже с заглавной, если это имя собственное (Тульский оружейный завод), но со строчной, если это общее прилагательное (тульский пряник).
- БРЕНДЫ И ПРОДУКТЫ ВСЕГДА с заглавной буквы (или как принято): Яндекс, Google, ChatGPT, Claude, Wildberries, Ozon, Авито, VK, ВКонтакте, Сбер, Тинькофф, 1С, Битрикс, Дзен, Telegram, YouTube. Никогда не пиши эти названия строчными.
- НИКОГДА не начинай заголовок H1/H2/H3 со строчной буквы. Это жёсткое правило: заголовок со строчной первой буквой считается невалидным.
...
### FORMATTING ELEMENTS
${showTable
  ? `- ТАБЛИЦЫ: Включи 1-2 сравнительные таблицы с реальными данными. ИСПОЛЬЗУЙ ТОЛЬКО GFM-СИНТАКСИС с вертикальными чертами и обязательной строкой-разделителем. Пример (точно такой формат, каждая ячейка обёрнута символами |):
...
ЗАПРЕЩЕНО: выравнивать столбцы пробелами или табуляцией без вертикальных черт - это не таблица, это сломанный текст. Каждая строка таблицы (включая заголовок и разделитель) должна начинаться и заканчиваться символом |.`
...
- НЕ ДОБАВЛЯЙ JSON-LD микроразметку FAQPage, теги <script type="application/ld+json"> или комментарии <!-- FAQ Schema --> в текст. Микроразметка генерируется отдельно по запросу пользователя.
...
${authorProfile ? `### [TONE BALANCE] (Баланс тона)
- СТРОГО СЛЕДУЙ тону и стилю автора из Блока А. Все остальные рекомендации по тону ВТОРИЧНЫ.
- Если в Блоке А указан конкретный стиль (ворчливый, академический, дерзкий и т.д.) — используй его в КАЖДОМ предложении.` : `### [TONE BALANCE] (Баланс тона)
...
### FINAL INSTRUCTION (АБСОЛЮТНЫЙ ПРИОРИТЕТ)
${authorProfile ? `- ПЕРЕЧИТАЙ Блок А перед написанием КАЖДОГО абзаца. Стиль, тон и инструкции автора из Блока А — это ЗАКОН. Любые конфликты с другими правилами — решай В ПОЛЬЗУ Блока А.
- Каждое предложение должно звучать так, как написал бы автор из Блока А. Если ты забыл стиль — вернись к Блоку А.` : `- Действуй как реальный человек-эксперт. Будь решительным, авторитетным и стилистическим.`}
...
- Учитывай SEO и удобство пользователя.`;
  ...
  // ═══ HARD OVERRIDE: пользовательский системный промт автора (наивысший приоритет, последняя инструкция) ═══
  const userAuthorInstruction = authorProfile?.type !== "preset" ? authorProfile?.system_instruction?.trim() : "";
  if (userAuthorInstruction) {
    systemPrompt += `\n\n═══════════════════════════════════════════════
### ABSOLUTE FINAL OVERRIDE — ИНСТРУКЦИЯ ПОЛЬЗОВАТЕЛЯ (ВЫСШИЙ ПРИОРИТЕТ)
═══════════════════════════════════════════════
Ниже — системный промт автора, заданный ПОЛЬЗОВАТЕЛЕМ напрямую.
Это ЗАКОН. Он перекрывает АБСОЛЮТНО ВСЕ правила выше (стиль, тон, структуру, форматирование, длину абзацев, запреты, рекомендации, SEO-инструкции — ВСЁ).
Если что-то в правилах выше противоречит этому промту — игнорируй те правила и следуй ЭТОМУ промту.
Применяй его к КАЖДОМУ предложению, КАЖДОМУ абзацу, КАЖДОМУ заголовку.

--- НАЧАЛО ПРОМТА ПОЛЬЗОВАТЕЛЯ ---
${userAuthorInstruction}
--- КОНЕЦ ПРОМТА ПОЛЬЗОВАТЕЛЯ ---

Перед написанием каждого абзаца — мысленно перечитай промт пользователя выше и убедись, что твой текст ему соответствует.`;
  }
...
export function buildNewArticleUserPrompt(
...
- Интегрируй рекомендации ЕСТЕСТВЕННО в соответствующие разделы, НЕ выделяй их отдельным блоком.\n`
  ...
  // ─── USER-PROVIDED PARAMETERS (highest priority, placed FIRST so the model anchors on them) ───
  const seoList = (seoKeywords || "").split(/[,;\n]+/).map(s => s.trim()).filter(Boolean);
  const userSeoBlock = seoList.length ? `\n🔴 SEO-КЛЮЧИ ОТ ПОЛЬЗОВАТЕЛЯ — ОБЯЗАТЕЛЬНО ВСТАВИТЬ КАЖДЫЙ:
${seoList.map((k, i) => `${i + 1}. "${k}" — минимум 1 упоминание в тексте. ОБЯЗАТЕЛЬНО склоняй ключ по падежу, числу и роду так, чтобы фраза была грамматически корректной. Точная форма из строки ключа НЕ ТРЕБУЕТСЯ — приоритет у норм русского языка, а не у дословного совпадения.`).join("\n")}
- ОДИН из ключей ДОЛЖЕН попасть в H1 или Title.
- МИНИМУМ ПОЛОВИНА ключей должна быть распределена по H2-заголовкам разделов.
- Каждый ключ повторяй минимум 1, максимум 3 раза. Запрещено собирать все ключи в одном абзаце.
- Если ключ не встречается ни в одной форме — статья считается невалидной.
- ЗАПРЕЩЕНО вставлять ключ в исходном падеже, если это ломает согласование ("заказать один круглые стикеры" — недопустимо; правильно: "заказать один круглый стикер").\n` : "";

  const userGeoBlock = geoLocation?.trim() ? `\n🔴 ГЕО-ПРИВЯЗКА: "${geoLocation}" — КРИТИЧНО, ИГНОРИРОВАТЬ ЗАПРЕЩЕНО:
- H1 ОБЯЗАН содержать "${geoLocation}" (или его прилагательное, например "московский", "питерский").
- Первый абзац введения ОБЯЗАН содержать "${geoLocation}".
- Топоним "${geoLocation}" должен встретиться в тексте минимум 4 раза (включая H1 и введение).
- Минимум 1 H2-раздел должен быть посвящен специфике региона "${geoLocation}" (климат, логистика, регулирование, местные особенности, цены, спрос).
- Используй синонимы и производные: "в ${geoLocation}", "местные жители", "в нашем регионе", "${geoLocation} и окрестности".
- Все примеры цен, контактов, законов, климата — давай в контексте "${geoLocation}", а не абстрактно.\n` : "";

  const userCustomBlock = customInstructions?.trim() ? `\n🔴 БИЗНЕС-ТРЕБОВАНИЯ КЛИЕНТА — НАИВЫСШИЙ ПРИОРИТЕТ (ВЫПОЛНИТЬ ВСЕ ДО ЕДИНОГО):
"""
${customInstructions}
"""
- Это инструкции ВЫШЕ конкурентов, выше LSI, выше структуры. При конфликте — выполняй ЭТИ требования.
- Каждый названный факт / бренд / цифра / условие ОБЯЗАН встретиться в финальном тексте.
- Подавай их как экспертное мнение или практический совет, не как рекламу (если в требованиях не указано иное).
- В заключении ОБЯЗАТЕЛЬНО сделай отсылку минимум к одному пункту из этих требований.\n` : "";

  const userParamsHeader = (seoList.length || geoLocation?.trim() || customInstructions?.trim())
    ? `═══════════════════════════════════════════
🔴 ПАРАМЕТРЫ ПОЛЬЗОВАТЕЛЯ — ОБРАБАТЫВАЙ ИХ В ПЕРВУЮ ОЧЕРЕДЬ
Эти инструкции имеют ПРИОРИТЕТ НАД ВСЕМ ОСТАЛЬНЫМ контекстом (конкуренты, LSI, общие правила).
Если что-то конфликтует — выигрывают эти параметры.
═══════════════════════════════════════════${userSeoBlock}${userGeoBlock}${userCustomBlock}
═══════════════════════════════════════════
КОНЕЦ БЛОКА ПОЛЬЗОВАТЕЛЬСКИХ ПАРАМЕТРОВ
═══════════════════════════════════════════
`
    : "";
...
- Впиши анкор как естественную часть предложения.\n`;
  ...
  // Strict ban on any links the user did not provide.
  const allowedUrls = [
    ...((miralinksLinks || []).filter(l => l.url && l.anchor).map(l => l.url)),
    ...((gogetlinksLinks || []).filter(l => l.url && l.anchor).map(l => l.url)),
    ...((anchorLinks || []).filter(l => l.url && l.anchor).map(l => l.url)),
  ];
  const noExternalLinksBlock = allowedUrls.length
    ? `\n🚫 ВНЕШНИЕ ССЫЛКИ - СТРОГИЙ ЗАПРЕТ:
- Разрешены ТОЛЬКО эти URL: ${allowedUrls.join(", ")}.
- НИКАКИХ других markdown-ссылок [текст](url) в тексте быть не должно.
- НЕ упоминай маркетплейсы, бренды и сайты (Ozon, Wildberries, Avito и подобные) в виде ссылок. Упоминать их можно только обычным текстом, без [...](...).
- Запрещены любые автогенерируемые ссылки, плейсхолдеры вида [Название](URL) и придуманные домены.\n`
    : `\n🚫 ВНЕШНИЕ ССЫЛКИ - СТРОГИЙ ЗАПРЕТ:
- НЕ вставляй НИКАКИХ markdown-ссылок [текст](url) в текст статьи.
- НЕ упоминай маркетплейсы, бренды и сайты (Ozon, Wildberries, Avito и подобные) в виде ссылок. Упоминать их можно только обычным текстом, без [...](...).
- Запрещены любые придуманные домены и плейсхолдеры вида [Название](URL).\n`;

  // Final compliance reminder (sandwich technique — повторяем приоритетные требования в конце).
  const finalReminder = (seoList.length || geoLocation?.trim() || customInstructions?.trim()) ? `

═══════════════════════════════════════════
🔴 ФИНАЛЬНЫЙ ЧЕК-ЛИСТ ПЕРЕД ВЫВОДОМ (выполни МОЛЧА перед тем, как начать писать):
${seoList.length ? `□ Каждый SEO-ключ из списка (${seoList.length} шт.) вписан минимум 1 раз.\n` : ""}${seoList.length ? "□ Один SEO-ключ попал в H1/Title.\n" : ""}${geoLocation?.trim() ? `□ "${geoLocation}" есть в H1 и в первом абзаце введения.\n` : ""}${geoLocation?.trim() ? `□ "${geoLocation}" упомянут в тексте не менее 4 раз.\n` : ""}${geoLocation?.trim() ? `□ Есть хотя бы один H2-раздел про региональную специфику "${geoLocation}".\n` : ""}${customInstructions?.trim() ? "□ ВСЕ пункты из бизнес-требований клиента выполнены, каждый факт/бренд/цифра упомянут.\n" : ""}Если хотя бы один пункт не выполнен — перепиши соответствующий раздел до того, как отдать ответ.
═══════════════════════════════════════════` : "";

  return `${userParamsHeader}КЛЮЧЕВОЕ СЛОВО: "${keyword.seed_keyword}"
ИНТЕНТ: ${keyword.intent || "informational"}
