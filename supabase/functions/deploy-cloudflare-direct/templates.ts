...
export function renderTemplate(ctx: RenderCtx): Record<string, string> {
  ...
  const emit = <T>(path: string, render: () => T): T | null =>
    ctx.renderPage ? ctx.renderPage(path, render) : (ctx.shouldRenderPage && !ctx.shouldRenderPage(path) ? null : render());
  ...
  for (const p of chromePosts) {
    ...
    if (html === null) delete files[postPath]; else files[postPath] = html;
  }
  ...
  for (const [path, build] of systemPages) {
    ...
    if (html === null) delete files[path]; else files[path] = html;
  }
