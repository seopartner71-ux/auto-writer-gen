...
5. brand (НЕ МОЖЕТ быть пустым): ПРОИЗВОДИТЕЛЬ товара - завод, марка, стандарт (например "Xingtai", "Apple", "ГОСТ 10299-80"). Это НЕ магазин. Если производитель не найден - поставь supplier_name, либо домен сайта.
6. supplier_name (НЕ МОЖЕТ быть пустым): ПРОДАВЕЦ - магазин или компания, которая продает товар на этой странице. Возьми название из подвала, шапки, блока "О компании" или из домена сайта. Один и тот же товар одного производителя могут продавать разные магазины, поэтому brand и supplier_name - разные поля и часто не совпадают.
