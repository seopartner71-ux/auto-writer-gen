...
import { Download, FileText, Loader2, AlertTriangle, RotateCcw, Copy, ExternalLink } from "lucide-react";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import { EcosystemFormat, Client } from "./types";
...
// Heavy multi-page docs (guide/expert_pdf) get an "open in new tab" shortcut
// instead of an inline iframe that blocks the UI while the PDF downloads.
const HEAVY_SLUGS = new Set(["expert_pdf", "guide"]);

export function DocumentPreviewModal({ open, onOpenChange, format, documentType, client, title }: Props) {
  ...
  const slug = documentType?.slug || format.format_type;
  const isHeavy = HEAVY_SLUGS.has(slug);
  ...
  const handleRetryPdf = async () => {
    ...
    try {
      ...
      toast.success("PDF пересобирается");
    } catch (e: any) {
      toast.error(e?.message || "Не удалось пересобрать PDF");
    } finally {
      setRetrying(false);
    }
  };
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success("Документ регенерируется");
      onOpenChange(false);
    } catch (e: any) {
      toast.error(e?.message || "Не удалось запустить регенерацию");
    } finally {
      ...
      <DialogContent className="max-w-4xl h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {title || documentType?.name || "Документ"}
          </DialogTitle>
        </DialogHeader>
            ...
            {format.pdf_url && !isPartial ? (
              isHeavy ? (
                <div className="flex flex-col items-center justify-center h-full text-center px-6 gap-3">
                  ...
                  <p className="text-sm font-medium">Многостраничный PDF ({documentType?.name || slug})</p>
                  <p className="text-xs text-muted-foreground max-w-md">
                    Открывайте в новой вкладке - так документ загрузится быстрее и без блокировки интерфейса.
                  </p>
                  <Button asChild size="sm">
                    <a href={format.pdf_url} target="_blank" rel="noreferrer">
                      <ExternalLink className="h-4 w-4 mr-2" /> Открыть в новой вкладке
                    </a>
                  </Button>
                </div>
              ) : (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center px-6 gap-3">
                ...
                <p className="text-xs text-muted-foreground">
                  Текст готов - переключитесь на вкладку Markdown или пересоберите PDF.
                </p>
