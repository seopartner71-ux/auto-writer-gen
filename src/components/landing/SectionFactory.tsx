...
import { Factory, Zap, CheckCircle, Clock, Globe, FileText, Sparkles, Shield } from "lucide-react";
import { useI18n } from "@/shared/hooks/useI18n";

/* ── Article data ── */
const articles = [
  { title: "Pool Service Guide", words: 2450, status: "live" },
  { title: "Crypto Tax 2025", words: 3102, status: "live" },
  { title: "Best CRM Tools", words: 2847, status: "live" },
  { title: "Как выбрать VPN", words: 1956, status: "live" },
  { title: "React vs Vue 2025", words: 2534, status: "live" },
  { title: "Home Insurance Tips", words: 2103, status: "live" },
  { title: "AI Writing Guide", words: 2891, status: "live" },
  { title: "SaaS Pricing Strategy", words: 1820, status: "live" },
];
  ...
  return (
    <span className="inline-block overflow-hidden h-[1.1em] relative align-bottom">
      <motion.span
        ...
        initial={{ y: `-${Math.floor(Math.random() * 5) * 10}%` }}
        animate={{ y: `-${digit * 10}%` }}
...
/* ── Marquee row ── */
function MarqueeRow({ direction = 1, speed = 35 }: { direction?: number; speed?: number }) {
  ...
  return (
    ...
    >
      <motion.div
        ...
        transition={{ duration: speed, repeat: Infinity, ease: "linear", ...(paused ? { duration: speed * 3 } : {}) }}
      >
        {doubled.map((art, i) => (
          <motion.div
            key={i}
            whileHover={{ scale: 1.06, zIndex: 10 }}
            className="shrink-0 w-[220px] rounded-xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-xl p-4 transition-shadow duration-300 hover:shadow-[0_10px_40px_rgba(139,92,246,0.12)] hover:border-white/[0.12] cursor-default"
          >
            ...
            <p className="text-[11px] font-semibold leading-tight truncate mb-2">{art.title}</p>
            <div className="flex items-center gap-1.5">
              <FileText className="h-2.5 w-2.5 text-muted-foreground/40" />
              <span className="text-[9px] font-tech text-muted-foreground/50">{art.words.toLocaleString()} words</span>
            </div>
          </motion.div>
        ))}
      </motion.div>
    ...
    <section ref={ref} className="relative min-h-screen flex items-center justify-center snap-start overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 max-w-6xl">
            ...
            {pipelineSteps.map((step, i) => {
              ...
              return (
                <div key={i} className="flex items-center">
                  ...
                  >
                    <div
                      className="w-11 h-11 rounded-xl flex items-center justify-center border border-white/[0.06]"
        ...
        <motion.div
          ...
          className="flex items-center justify-center gap-8 sm:gap-14 mb-8"
        >
          ...
          <div className="text-center">
            <p className="text-4xl sm:text-5xl font-black text-emerald-400" style={{ letterSpacing: "-0.06em" }}>
              {inView ? <OdometerNumber value={92} delay={0.4} /> : "0"}
