    ...
    <section className="relative py-36 overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 max-w-6xl">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          >
            <span className="text-xs font-mono text-emerald-400/70 tracking-widest uppercase mb-5 block">
              {t("stealth.tag")}
            </span>
            <h2 className="text-3xl md:text-5xl font-extrabold tracking-[-0.03em] text-white mb-7">
              {t("stealth.title")}
            </h2>
            <p className="text-base text-foreground/55 leading-[1.8] mb-10 max-w-lg">
              {t("stealth.body")}
            </p>
            ...
            <div className="space-y-5">
              {[
                ...
                <div key={i} className="flex items-start gap-3">
                  <p.icon className="h-5 w-5 text-emerald-400/80 mt-0.5 shrink-0" />
                  <span className="text-sm text-foreground/70 leading-relaxed">{p.text}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          >
            <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-1.5">
              <div className="rounded-xl bg-[#08080e]/90 p-10 min-w-[300px]">
                <p className="text-center text-[11px] font-mono text-foreground/35 uppercase tracking-widest mb-8">
                  Stealth Engine™
                </p>
                ...
                <div className="flex justify-center mb-8">
                  <div className="relative w-[170px] h-[170px]">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      ...
                      <span className="text-[11px] font-mono text-foreground/40 mt-1">Human Score</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 text-center">
                    <p className="text-2xl font-bold text-primary/90" style={{ letterSpacing: "-0.04em" }}>{perplexity.toFixed(1)}</p>
                    <p className="text-[10px] font-mono text-foreground/35 mt-1">Perplexity</p>
                  </div>
                  <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 text-center">
                    <p className="text-2xl font-bold text-[#3b82f6]/90" style={{ letterSpacing: "-0.04em" }}>{burstiness.toFixed(1)}</p>
                    <p className="text-[10px] font-mono text-foreground/35 mt-1">Burstiness</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 justify-center">
                  {["Originality.ai", "GPTZero", "Copyleaks", "Turnitin"].map((d, i) => (
                    <span key={i} className={`text-[10px] font-mono px-2.5 py-1 rounded-full border ${
                      score >= 90
                        ? "border-emerald-500/20 bg-emerald-500/[0.06] text-emerald-400/90"
                        : "border-white/[0.06] bg-white/[0.02] text-foreground/35"
                    }`}>
                      {score >= 90 ? "✓" : "○"} {d}
                    </span>
                  ))}
