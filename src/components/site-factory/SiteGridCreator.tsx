                  ...
                  <div className="space-y-1 sm:col-span-2">
                    ...
                    <select
                      value={spec.homepageStyle}
                       onChange={(e) => updateSpec(idx, { homepageStyle: e.target.value as "landing" | "magazine" | "news" | "minimal" | "dark" | "local" | "expert" })}
                      disabled={running}
                      className="h-8 w-full rounded-md border border-input bg-background px-2 text-xs disabled:opacity-50"
                    >
                      ...
                      <option value="expert">Экспертный (E-E-A-T, авторские публикации)</option>
                    </select>
                  </div>
                </div>
              </div>
