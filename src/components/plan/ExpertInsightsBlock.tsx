...
const EEAT_COLORS: Record<string, string> = { experience: "bg-info/15 text-info border-info/30", expertise: "bg-warning/15 text-warning border-warning/30", authority: "bg-primary/15 text-primary border-primary/30", trust: "bg-success/15 text-success border-success/30" };
...
export function ExpertInsightsBlock({ keywordId, onAddToOutline }: Props) {
  ...
  const analyze = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await supabase.functions.invoke("analyze-content-gaps", { body: { keyword_id: keywordId }, headers: { Authorization: `Bearer ${session.access_token}` } });
    ...
    return (
      <Card className="bg-card border-border">
        <CardContent className="py-6 flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground text-center">{t("expert.deepAnalysis")}</p>
          <Button onClick={() => analyze.mutate()} disabled={analyze.isPending} className="gap-2" variant="outline">
            {analyze.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
            {analyze.isPending ? t("expert.analyzing") : t("expert.findGaps")}
          </Button>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className="space-y-4">
            ...
            <div key={i} className="flex items-start gap-3 rounded-md bg-primary/5 border border-primary/10 p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-1.5 mt-1.5">
                  <Badge variant="outline" className={`text-[9px] ${EEAT_COLORS[insight.eeat_category]}`}>{EEAT_LABELS[insight.eeat_category]}</Badge>
                  {insight.impact === "high" && <Badge className="text-[9px] bg-success/15 text-success border-success/30">{t("expert.highImpact")}</Badge>}
                </div>
              </div>
            </div>
          ))}
      ...
      <Card className="bg-card border-destructive/20">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm flex items-center gap-2">
            <Lightbulb className="h-4 w-4 text-destructive" />Missing Topics (Gap)
            <Badge className="ml-auto text-[10px] bg-destructive/15 text-destructive border-destructive/30">{analysis.missing_topics.length}</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          {analysis.missing_topics.map((topic, i) => (
            <div key={i} className="rounded-md bg-destructive/5 border border-destructive/10 p-3 group">
              <div className="flex items-start justify-between gap-2">
                <p className="text-sm font-medium">{topic.topic}</p>
                {onAddToOutline && <Button variant="ghost" size="icon" className="h-6 w-6 opacity-0 group-hover:opacity-100 shrink-0" onClick={() => onAddToOutline(topic.topic, "h2")}><ArrowRight className="h-3 w-3" /></Button>}
              </div>
              <p className="text-xs text-muted-foreground mt-1">{topic.why_important}</p>
              <Badge variant="outline" className={`text-[9px] mt-1.5 ${EEAT_COLORS[topic.eeat_aspect]}`}>{EEAT_ICONS[topic.eeat_aspect]}<span className="ml-1">{EEAT_LABELS[topic.eeat_aspect]}</span></Badge>
            </div>
          ))}
        </CardContent>
      </Card>
      ...
      <Card className="bg-card border-border">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm flex items-center gap-2">
            <Users className="h-4 w-4 text-muted-foreground" />{t("expert.commonTopics")}
            <Badge variant="secondary" className="ml-auto text-[10px]">{analysis.common_topics.length}</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-1.5">
          {analysis.common_topics.map((topic, i) => (
            <div key={i} className="flex items-center gap-2 rounded-md bg-muted/40 px-3 py-2 group">
              ...
              <Badge variant="outline" className={`text-[9px] ${IMPORTANCE_COLORS[topic.importance]}`}>
                {topic.importance === "critical" ? t("expert.critical") : topic.importance === "high" ? t("expert.important") : t("expert.medium")}
              </Badge>
              <span className="text-[10px] text-muted-foreground">{topic.coverage_count}/10</span>
              {onAddToOutline && <Button variant="ghost" size="icon" className="h-5 w-5 opacity-0 group-hover:opacity-100" onClick={() => onAddToOutline(topic.topic, "h2")}><ArrowRight className="h-3 w-3" /></Button>}
            </div>
          ))}
        </CardContent>
      </Card>
      ...
      <Card className="bg-card border-border">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm flex items-center gap-2">
            <Star className="h-4 w-4 text-warning" />{t("expert.uniqueTopics")}
            <Badge variant="secondary" className="ml-auto text-[10px]">{analysis.unique_topics.length}</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-1.5">
          {analysis.unique_topics.map((topic, i) => (
            <div key={i} className="flex items-center gap-2 rounded-md bg-muted/40 px-3 py-2 group">
              ...
              <Badge variant="outline" className={`text-[9px] ${DIFF_COLORS[topic.differentiation_value]}`}>
                {topic.differentiation_value === "high" ? t("expert.high") : topic.differentiation_value === "medium" ? t("expert.mediumDiff") : t("expert.low")}
              </Badge>
              {onAddToOutline && <Button variant="ghost" size="icon" className="h-5 w-5 opacity-0 group-hover:opacity-100" onClick={() => onAddToOutline(topic.topic, "h2")}><ArrowRight className="h-3 w-3" /></Button>}
            </div>
          ))}
      ...
      <Button variant="outline" size="sm" className="w-full gap-2" onClick={() => analyze.mutate()} disabled={analyze.isPending}>
        {analyze.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3" />}{t("expert.reanalyze")}
      </Button>
    </div>
  );
}
